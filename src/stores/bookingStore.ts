...
export const useBookingStore = defineStore('bookingStore', () => {
  ...
  const availableDaysFormat = computed(() =>
    availableDays.value.map((date) => getDateLabel(date, restaurant.value?.current_day ?? '2025-04-04')),
  )

  async function loadData() {
    const url = 'https://hh.frontend.ark.software/api/booking'
    try {
      isLoader.value = true
      ...
      allZones.value = [...new Set(restaurant.value.tables.map((t: Table) => t.zone))].sort((a, b) => {
